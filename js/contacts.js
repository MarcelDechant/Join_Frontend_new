...
async function saveContact() {
    ...
    let insertIndex = 0;
    while (insertIndex < state.contacts.length && state.contacts[insertIndex]['add_name'].localeCompare(name) < 0) {
        insertIndex++;
    }
        ...
        allContacts.forEach((contact, index) => {
            if (contact['add_name']) {
               ...
               <div>
                    ...
                    <div class="email">${contact['add_email']}</div>
                    <div class="phone d-none">${contact['add_phone']}</div>
                 </div>
                </div>`;
...
async function deleteContact(name) {
    let indexToDelete = state.contacts.findIndex(contact => contact['add_name'] === name);
...
async function saveEditedContact(contactId) {
    ...
    if (indexToEdit !== -1) {
        ...
        state.contacts[indexToEdit]['add_name'] = editedName;
        state.contacts[indexToEdit]['add_email'] = editedEmail;
        state.contacts[indexToEdit]['add_phone'] = editedPhone;
